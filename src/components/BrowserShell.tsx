import React, { useState, useEffect } from 'react';
import { Shield, Lock, RotateCw, ChevronLeft, ChevronRight, Share2, Plus, X, Mail, Copy, Check } from 'lucide-react';
import { m, AnimatePresence } from 'framer-motion';

interface BrowserShellProps {
  children: React.ReactNode;
  playClick?: () => void;
  playType?: () => void;
}

// Brand SVG Icons for sharing platforms
const WhatsAppIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L0 24l6.335-1.662c1.746.953 3.71 1.455 5.703 1.458h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
  </svg>
);

const LinkedInIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
  </svg>
);

const DiscordIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994a.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.873-.894.077.077 0 0 1-.008-.128c.126-.093.252-.19.372-.287a.075.075 0 0 1 .077-.011c3.92 1.793 8.18 1.793 12.061 0a.073.073 0 0 1 .078.009c.12.099.246.195.373.289a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.894.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.156-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.156 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.156-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.156 2.418z" />
  </svg>
);

const InstagramIcon = ({ size = 18 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <rect x="2" y="2" width="20" height="20" rx="5" ry="5" />
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
    <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" />
  </svg>
);

export function BrowserShell({ children, playClick, playType }: BrowserShellProps) {
  const [isShareOpen, setIsShareOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const [showBookmarkToast, setShowBookmarkToast] = useState(false);

  useEffect(() => {
    if (!showBookmarkToast) return;
    const timer = setTimeout(() => {
      setShowBookmarkToast(false);
    }, 4500);
    return () => clearTimeout(timer);
  }, [showBookmarkToast]);

  const handleBookmarkClick = () => {
    if (playClick) playClick();
    setShowBookmarkToast(true);
  };

  const portfolioUrl = "https://ebin-reji.vercel.app";

  const handleCopy = async () => {
    if (playClick) playClick();
    try {
      await navigator.clipboard.writeText(portfolioUrl);
      setCopied(true);
      setTimeout(() => setCopied(false), 2000);
    } catch (err) {
      console.error('Failed to copy: ', err);
    }
  };

  const shareOptions = [
    {
      name: 'WhatsApp',
      icon: <WhatsAppIcon size={13} />,
      url: `https://api.whatsapp.com/send?text=Check%20out%20Ebin%20Reji's%20portfolio:%20${encodeURIComponent(portfolioUrl)}`
    },
    {
      name: 'Gmail',
      icon: <Mail size={13} />,
      url: `mailto:?subject=Ebin%20Reji%20Portfolio&body=Check%20out%20Ebin%20Reji's%20portfolio%20at%20${encodeURIComponent(portfolioUrl)}`
    },
    {
      name: 'LinkedIn',
      icon: <LinkedInIcon size={13} />,
      url: `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(portfolioUrl)}`
    },
    {
      name: 'Discord',
      icon: <DiscordIcon size={13} />,
      url: `https://discord.com/channels/@me`
    },
    {
      name: 'Instagram',
      icon: <InstagramIcon size={13} />,
      url: `https://www.instagram.com/`
    }
  ];

  return (
    <div className="relative min-h-screen w-full flex flex-col p-4 md:p-8 xl:p-12 z-10 box-border">
      {/* Outer Window Border Container */}
      <div className="w-full max-w-7xl mx-auto flex flex-col bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[22px] shadow-2xl transition-all duration-500">
        
        {/* Browser Top Header Bar */}
        <div className="h-14 w-full flex items-center justify-between px-6 border-b border-[var(--border-color)] bg-[var(--bg-card)] select-none shrink-0 rounded-t-[22px]">
          
          {/* OS Window Controls (Red, Yellow, Green circles) */}
          <div className="flex items-center gap-2 w-1/4">
            <div className="w-3.5 h-3.5 rounded-full bg-[#FF5F56] border border-[#E0443E] hover:opacity-80 transition-opacity cursor-pointer" />
            <div className="w-3.5 h-3.5 rounded-full bg-[#FFBD2E] border border-[#DEA123] hover:opacity-80 transition-opacity cursor-pointer" />
            <div className="w-3.5 h-3.5 rounded-full bg-[#27C93F] border border-[#1AAB29] hover:opacity-80 transition-opacity cursor-pointer" />
          </div>

          {/* Browser Navigation & Address Bar */}
          <div className="flex items-center gap-3 w-2/4 justify-center">
            {/* Nav arrows (hidden on small screens) */}
            <div className="hidden sm:flex items-center gap-2 text-[var(--text-secondary)]">
              <ChevronLeft size={16} className="opacity-40 cursor-not-allowed" />
              <ChevronRight size={16} className="opacity-40 cursor-not-allowed" />
              <RotateCw 
                size={14} 
                className="hover:text-[var(--text-primary)] transition-colors cursor-pointer" 
                onClick={playClick}
                onMouseEnter={playType}
              />
            </div>

            {/* Address Bar */}
            <div className="flex items-center gap-2 px-4 py-1.5 rounded-full bg-[var(--bg-primary)] border border-[var(--border-color)] text-xs text-[var(--text-secondary)] w-64 md:w-80 justify-between select-none">
              <div className="flex items-center gap-1.5 truncate">
                <Lock size={12} className="text-emerald-500 shrink-0" />
                <span className="font-mono truncate tracking-tight text-[var(--text-primary)]">ebin-reji.vercel.app</span>
              </div>
              <Shield size={12} className="text-[var(--text-secondary)] shrink-0" />
            </div>
          </div>

          {/* Right Action Icons */}
          <div className="flex items-center gap-4 w-1/4 justify-end text-[var(--text-secondary)]">
            <Share2 
              size={16} 
              className="hover:text-[var(--text-primary)] transition-colors cursor-pointer" 
              onClick={() => {
                if (playClick) playClick();
                setIsShareOpen(true);
              }}
              onMouseEnter={playType}
            />
            <Plus 
              size={16} 
              className="hover:text-[var(--text-primary)] transition-colors cursor-pointer"
              onClick={handleBookmarkClick}
              onMouseEnter={playType}
            />
          </div>
        </div>

        {/* Scrollable Browser Body */}
        <div className="w-full flex flex-col bg-[var(--bg-primary)] relative rounded-b-[22px] overflow-hidden">
          {children}
        </div>
      </div>

      {/* Share Modal Overlay */}
      <AnimatePresence>
        {isShareOpen && (
          <div 
            className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4" 
            onClick={() => {
              if (playClick) playClick();
              setIsShareOpen(false);
            }}
          >
            <m.div 
              initial={{ opacity: 0, scale: 0.96, y: 8 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.96, y: 8 }}
              transition={{ duration: 0.2, ease: 'easeOut' }}
              className="w-full max-w-md bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[22px] p-8 md:p-10 relative shadow-2xl flex flex-col gap-8 [will-change:transform]"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Modal Close Icon */}
              <button 
                onClick={() => {
                  if (playClick) playClick();
                  setIsShareOpen(false);
                }}
                onMouseEnter={playType}
                className="absolute top-6 right-6 p-1.5 rounded-full border border-[var(--border-color)] bg-[var(--bg-elevated)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--text-secondary)] transition-all cursor-pointer z-10"
                aria-label="Close share window"
              >
                <X size={13} />
              </button>

              <div className="flex flex-col gap-6">
                {/* Header */}
                <div>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--text-secondary)] block mb-1">
                    Share
                  </span>
                  <h2 className="font-serif text-3xl font-light tracking-tight text-[var(--text-primary)] m-0">
                    Share portfolio
                  </h2>
                </div>

                {/* Minimalist URL Display and Copy Section */}
                <div className="flex items-center justify-between border-b border-[var(--border-color)] pb-3 pt-2">
                  <span className="font-mono text-sm tracking-tight text-[var(--text-primary)] select-text truncate mr-4">
                    {portfolioUrl}
                  </span>
                  <button
                    onClick={handleCopy}
                    onMouseEnter={playType}
                    className="group inline-flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-widest text-[var(--text-primary)] hover:text-[var(--text-secondary)] transition-colors duration-300 cursor-pointer shrink-0"
                  >
                    {copied ? <Check size={11} className="text-emerald-500" /> : <Copy size={11} />}
                    <span>{copied ? 'Copied' : 'Copy'}</span>
                  </button>
                </div>

                {/* Share options */}
                <div className="flex flex-col gap-1 pt-2">
                  <span className="font-mono text-[10px] uppercase tracking-widest text-[var(--text-secondary)] block mb-2">
                    Share via
                  </span>
                  {shareOptions.map((opt) => (
                    <a
                      key={opt.name}
                      href={opt.url}
                      target="_blank"
                      rel="noopener noreferrer"
                      onClick={() => {
                        if (playClick) playClick();
                      }}
                      onMouseEnter={playType}
                      className="group flex items-center justify-between border-b border-[var(--border-color)] py-3 text-[var(--text-primary)] hover:text-[var(--text-secondary)] transition-colors duration-300 cursor-pointer"
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-[var(--text-primary)] group-hover:text-[var(--text-secondary)] transition-colors duration-300 flex items-center justify-center">
                          {opt.icon}
                        </span>
                        <span className="font-mono text-xs uppercase tracking-widest">
                          {opt.name}
                        </span>
                      </div>
                      <span className="font-mono text-xs transform group-hover:translate-x-1 transition-all duration-300 opacity-60 group-hover:opacity-100">
                        →
                      </span>
                    </a>
                  ))}
                </div>
              </div>
            </m.div>
          </div>
        )}
      </AnimatePresence>

      {/* Bookmark Helper Toast */}
      <AnimatePresence>
        {showBookmarkToast && (
          <m.div
            initial={{ opacity: 0, y: 20, scale: 0.95 }}
            animate={{ opacity: 1, y: 0, scale: 1 }}
            exit={{ opacity: 0, y: 15, scale: 0.95 }}
            transition={{ duration: 0.25, ease: 'easeOut' }}
            className="fixed bottom-6 right-6 md:bottom-10 md:right-10 z-55 bg-[var(--bg-card)] border border-[var(--border-color)] rounded-xl py-3 px-4 shadow-2xl backdrop-blur-md flex items-center gap-3.5 max-w-sm pointer-events-auto [will-change:transform]"
          >
            <div className="w-1.5 h-1.5 rounded-full bg-[var(--text-primary)] animate-pulse shrink-0" />
            <div className="flex flex-col gap-0.5 min-w-0">
              <span className="font-mono text-[9px] uppercase tracking-widest text-[var(--text-secondary)]">
                Shortcut
              </span>
              <span className="font-sans text-xs text-[var(--text-primary)] leading-normal">
                Press <kbd className="font-mono bg-[var(--bg-elevated)] border border-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] text-[var(--text-primary)] font-bold mx-0.5">{typeof window !== 'undefined' && /Mac|iPod|iPhone|iPad/.test(navigator.userAgent) ? '⌘' : 'Ctrl'}</kbd> + <kbd className="font-mono bg-[var(--bg-elevated)] border border-[var(--border-color)] px-1.5 py-0.5 rounded text-[10px] text-[var(--text-primary)] font-bold mx-0.5">D</kbd> to bookmark this page.
              </span>
            </div>
            <button
              onClick={() => setShowBookmarkToast(false)}
              className="text-[var(--text-secondary)] hover:text-[var(--text-primary)] ml-2 transition-colors cursor-pointer shrink-0"
              aria-label="Close notification"
            >
              <X size={12} />
            </button>
          </m.div>
        )}
      </AnimatePresence>
    </div>
  );
}
